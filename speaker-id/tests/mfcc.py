...
def mfcc(signal, sample_rate=SR, frame_length=FRAME_LENGTH, frame_step=FRAME_STEP, num_mfcc=N_MFCC, num_mel_filters=MEL_FILTERS, target_frames=TARGET_FRAMES):
    ...
    if M.shape[1] > target_frames:
        M = M[:, :target_frames]
    elif M.shape[1] < target_frames:
        pad_width = target_frames - M.shape[1]
...
def melspectogram(y, sr=16000, n_fft=2048, hop_length=512, n_mels=128, fmin=0.0, fmax=None):
    if not isinstance(y, np.ndarray):
        raise ValueError("Input signal must be a 1D numpy array.")
    if fmax is None:
        fmax = sr / 2
    ...
    # stft = scipy.signal.stft(y,nfft=n_fft)
    # print('mel_basis',mel_basis.shape)
    # power_spec = power_spectogram(stft)
    # print('power spectogram',stft)
    # mel_spectrogram = np.einsum("...ft,mf->...mt", S, mel_basis, optimize=True)
